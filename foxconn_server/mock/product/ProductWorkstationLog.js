...
const ProductWorkstationLogService=require('../../api/product/ProductWorkstationLogService')
 let id=1
 let status=['工作','空闲','异常']
 let good=5000
 let deafultData=[
...
function upDateLogs(){
    ProductWorkstationLogService.count().then(res=>{
        if(res===16){
            good+=GetRandomNum(3,5)
            ProductWorkstationLogService.update({
                ID:id,
            ...
            if(id===17){
                ...
                ProductWorkstationLogService.destroyByCondition({ID:{$gt:0}}).then(
                    deafultData.forEach(item=>{
                        ProductWorkstationLogService.create(item)
                    })
            )
        }
    })
